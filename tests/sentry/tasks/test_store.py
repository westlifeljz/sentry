...
@pytest.fixture
def mock_event_tempstore():
    with mock.patch("sentry.tasks.store.event_tempstore") as m:
        yield m
...
def test_symbolicate_event_call_process_inline(
    ...
    mock_event_tempstore.get.return_value = data
    mock_event_tempstore.store.return_value = "e:1"
    ...
    ((_, (event,), _),) = mock_event_tempstore.store.mock_calls
...
def test_process_event_mutate_and_save(
    default_project, mock_event_tempstore, mock_save_event, register_plugin
):
    register_plugin(BasicPreprocessorPlugin)
...
def test_process_event_no_mutate_and_save(
    ...
    assert mock_event_tempstore.store.call_count == 0
